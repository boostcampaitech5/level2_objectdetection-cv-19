...
from ultralytics.yolo.utils.callbacks.wb import callbacks as wandb_callback
...
class YOLO:
    ...
    def _load(self, weights: str, task=None):
        ...
        if suffix == ".pt":
            self.model, self.ckpt = attempt_load_one_weight(weights)  # model, 생성된 model의 학습 cfg
            self.task = self.model.args["task"]  # detect
            self.overrides = self.model.args = self._reset_ckpt_args(
                self.model.args
            )  # {'task': 'detect', 'data': 'coco.yaml', 'imgsz': 640, 'single_cls': False}
            self.ckpt_path = self.model.pt_path  # yolov8n.pt
        else:
    ...
    def train(self, **kwargs):
        ...
        """
        self._check_is_pytorch_model()
        if self.session:  # Ultralytics HUB session, not in our case
            if any(kwargs):
        ...
        self.task = overrides.get("task") or self.task
        self.trainer = TASK_MAP[self.task][1](overrides=overrides, _callbacks=self.callbacks)  # DetectionModel
        for k in wandb_callback:
            self.trainer.add_callback(k, wandb_callback[k])  # wandb callback 추가
        ...
        self.trainer.train()
        # Update model and cfg after training, 학습 종료 후에 logging
        if RANK in (-1, 0):
