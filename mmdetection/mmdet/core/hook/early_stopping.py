...
class EarlyStoppingHook(Hook):
    ...
    Args:
        ...
        patiHOence: number of checks with no improvement
    ...
    def __init__(
        ...
        assert isinstance(patience, int) and patience > 0, "patience must be positive integer"
    ...
    def before_run(self, runner):
        ...
        self.wait_count = runner.meta["hook_msgs"].get("wait_count", self.wait_count)
        self.best_score = runner.meta["hook_msgs"].get("early_stop_best_score", self.best_score)
        self.should_stop = runner.meta["hook_msgs"].get("should_stop", self.should_stop)
    ...
    def after_val_epoch(self, runner):
        eval_results = runner.meta.get("eval_results")
        ...
        self.duplicate = True
        runner.log_buffer.average()
        if self.monitor in eval_results:
            self._run_early_stopping_check(runner, eval_results)
        else:
            self._run_early_stopping_check(runner, runner.log_buffer.output)
    ...
    def _validate_condition_metric(self, runner, logs: Dict[str, float]) -> bool:
    ...
    def _run_early_stopping_check(self, runner, logs: Dict[str, float]):
        if not self._validate_condition_metric(runner, logs):  # short circuit if metric not present
            return
        if type(logs[self.monitor]) == float:
    ...
    def _evaluate_stopping_criteria(self, current: float):
        ...
        if self.check_finite and not np.isfinite(current):
            ...
            self.wait_count += 1
            reason = (
                f"Metric {self.monitor} unimproved."
                f" {self.monitor} = {current:.3f}"
                f" Best score is {self.best_score:.3f}"
            )
        ...
        print("self.wait_count : " + str(self.wait_count))
    ...
    def _improvement_message(self, current: np.float) -> str:
        ...
        if np.isfinite(self.best_score):
            ...
            msg = f"Metric {self.monitor} improved. New best score:" f" {current:.3f}"
        return msg
