...
def build_batch_data_loader(dataset, sampler, total_batch_size, *, aspect_ratio_grouping=False, num_workers=0):
    ...
    """
    world_size = get_world_size() # used for multi-gpu case
    assert (
    ...
    if aspect_ratio_grouping:
        data_loader = torch.utils.data.DataLoader(
            ...
            worker_init_fn=worker_init_reset_seed, # 어떤 worker를 가져올 지.
        )  # yield individual mapped dict
        return AspectRatioGroupedDataset(data_loader, batch_size) 
        # aspect ratio가 1 이상인 그룹끼리, 이하인 그룹끼리 반환 -> less padding이 필요하므로 속도 이점
    else:
...
# cfg의 setting으로부터 dataset load
def _train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None):
    if dataset is None:
        dataset = get_detection_dataset_dicts(
            cfg.DATASETS.TRAIN,                                                     # json path
        ...
        _log_api_usage("dataset." + cfg.DATASETS.TRAIN[0]) # logging training dataset

    if mapper is None:
        mapper = DatasetMapper(cfg, True) # use default mapper class
...
def build_detection_train_loader(
    ...
    if sampler is None:
        sampler = TrainingSampler(len(dataset))      # 왜 TrainingSampler로 hard-coding되어 있는거지??
